...
API_BASE_URL = os.getenv("API_BASE_URL", "http://61.71.33.195:52922")
SEGMENT_URL = "http://61.71.33.195:52922/segment"
...
def call_segment_api(file_bytes: bytes, filename: str, timeout: int = 300) -> bytes:
    """POST file to SEGMENT_URL as multipart/form-data, return raw PLY bytes on success."""
...
with st.container(border=True):
    ...
    if st.session_state.upload_state == "idle":
        ...
        with st.container(border=True):
            ...
            with col1:
                st.markdown(
                    "<p style='margin-top: 8px; font-weight: 600;'>Email Notifications</p>",
                    unsafe_allow_html=True,
                )
            with col2:
            ...
            if st.button("Start", type="primary", use_container_width=True):
                ...
                with open(save_path, "wb") as f:
                    ...
                    while True:
                        ...
                        progress = (
                            min(bytes_written / file_size, 1.0) if file_size > 0 else 1.0
                        )
                        speed_mb = (
